...
void setup() {
  ...
  //sets pin 5 to input
  //refers to the push button connected to the kettle bell
  pinMode(5, INPUT);
}
...
void loop() {
  ...
  //if the input from the push button is low, the arduino will create and send a character array and then print confirmation of the sent message to the monitor
  ...
  if(digitalRead(5) == LOW){
    const char text[] = "dumbon";
    radio.write(&text, sizeof(text));
    Serial.println(text);
  }

  //if the input from the push button is high, the arduino will create and send a character array and then print confirmation of the sent message to the monitor
  //Important note, this doesn't work with a string, it only works with character arrays. You can reconvert it to a string in the receiver code by 
  //creating a new string variable and setting it equal to the sent character array.
  if(digitalRead(5) == HIGH){
    const char text[] = "dumboff";
    radio.write(&text, sizeof(text));
    Serial.println(text);
  }
}
